...
var _ = Describe(fmt.Sprintf("%sFilterBy", getAppString()), func() {
	...
	It("Should filter by first number", func() {
		By("Equaling 1")
		i1, e1 := AppFilterBy(items, func(item cfclient.App) (bool, error) {
		...
		By("Equaling 2")
		i2, e2 := AppFilterBy(items, func(item cfclient.App) (bool, error) {
			s := strings.Split(item.Guid, "-")
			return s[1] == "2", nil
		})
		Expect(e2).To(BeNil())
